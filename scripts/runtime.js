...
import { buildGoblinEngineeringPacks } from "./data/packs.js";
import { readPackSettings } from "./settings.js";

let activeRuntime = null;

export function initializeGoblinEngineering(forge, { gameRef = globalThis.game } = {}) {
  if (!forge?.extensions?.forModule) {
    throw new TypeError("PF2E Critical Forge: Goblin Engineering requires the Critical Forge public extension API.");
  }
  ...
  const runtime = Object.freeze({
    ...
    refresh: () => refreshGoblinEngineeringPacks({ gameRef }),
...
export function refreshGoblinEngineeringPacks({ gameRef = globalThis.game } = {}) {
  if (!activeRuntime?.extension) return null;
  return registerCurrentPacks(activeRuntime.extension, gameRef, { replace: true });
}

export function getGoblinEngineeringRuntime() {
  return activeRuntime;
}

function registerCurrentPacks(extension, gameRef, { replace = false } = {}) {
  const packs = buildGoblinEngineeringPacks(readPackSettings(gameRef));
