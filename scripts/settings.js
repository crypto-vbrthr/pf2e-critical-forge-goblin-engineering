...
import { GOBLIN_PACK_CONFIGS } from "./data/packs.js";
...
export function registerSettings(gameRef = globalThis.game) {
  ...
  for (const config of GOBLIN_PACK_CONFIGS) {
    settings.register(MODULE_ID, config.settingKey, {
      name: `PF2E_GOBLIN_ENGINEERING.Settings.${config.settingToken}.Name`,
      hint: `PF2E_GOBLIN_ENGINEERING.Settings.${config.settingToken}.Hint`,
      scope: "world",
